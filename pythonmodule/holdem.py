...
class HoldemArena:
    ...
    def __init__(self, smallest_chip_amount):
        self.c_holdem_table = create_new_table(self.seat_number,  smallest_chip_amount)

    def __del__(self):
        delete_table_and_players(c_holdem_table)

    def add_player_clockwise(self,  player_name, starting_money,  bot_type):
        if bot_type == None:
            add_a_human_opponent(self.c_holdem_table,  player_name,  starting_money)
        elif bot_type.lower() in valid_bot_types:
            add_a_strategy_bot(self.c_holdem_table,  player_name,  starting_money,  bot_type[0])
        else:
    ...
    def pot(self):
        ...
        return get_pot_size(self._arena.c_holdem_table[0])
    
    @property
    def bet_to_call(self):
        """Get the largest bet so far this round."""
        return get_bet_to_call(self._arena.c_holdem_table[0])
        
    def get_previous_rounds_pot(self):
        """Get the pot size from just after the previous betting round ended."""
        return get_previous_rounds_pot_size(self._arena.c_holdem_table[0])
...
class HoldemArenaPlayer:
    ...
    def calculate_bet_decision(self):
        return get_bot_bet_decision(_arena.c_holdem_table[0], self.seat_number)
    

    def _getmoney(self):
        return get_money(self._arena.c_holdem_table[0], self.seat_number)
    def _setmoney(self,  new_money):
        set_money(self._arena.c_holdem_table[0], self.seat_number, new_money)
    money = property(_getmoney, _setmoney)

    @property
    def current_round_bet(self):
        """Get the current bet that the player has made this round."""
        return get_current_round_bet(self._arena.c_holdem_table[0], self.seat_number)

    @property
    def previous_rounds_bet(self):
        """Get the total bets that the player has made in all previous rounds."""
        return get_previous_rounds_bet(self._arena.c_holdem_table[0], self.seat_number)
...
class HoldemArenaCards:
    ...
    c_holdem_cardset = None

    def __init__(self):
        self.c_holdem_cardset = create_new_cardset()

    def __del__(self):
        delete_cardset(self.c_holdem_cardset)

    def append_card(self,  card_str):
        self.c_holdem_cardset = append_card_to_cardset(self.c_holdem_cardset,  card_str[0],  card_str[1])
